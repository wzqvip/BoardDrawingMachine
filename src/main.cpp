...
#define SPOOL_DIAMETER (36)    //线轴直径mm
#define SPOOL_CIRC (SPOOL_DIAMETER * 3.1416)  //线轴周长 35*3.14=109.956
#define TPS \
  (SPOOL_CIRC / STEPS_PER_TURN)  //步进电机步距，最小分辨率 每步线绳被拉动的距离
                                 // 0.053689mm

#define step_delay 1  //步进电机每步的等候时间 （微妙）
#define TPD \
  100  //转弯等待时间（毫秒），由于惯性笔会继续运动，暂定等待笔静止再运动。
...
/* 垂直距离的参数：
正值在画板下放，理论上只要画板够大可以无限大，负值区域在笔（开机前）的上方
*/
#define LIMYMAX (400)  // y轴最大值 画板最下方
#define LIMYMIN (400)  // y轴最小值 画板最上方
                // 左右两线的固定点到笔的垂直距离，尽量测量摆放准确，误差过大会有畸变
                //值缩小画图变瘦长，值加大画图变矮胖
//抬笔舵机的角度参数  具体数值要看摆臂的安放位置，需要调节
#define PEN_UP_ANGLE 37    //抬笔
#define PEN_DOWN_ANGLE 65  //落笔
//上面是需要调节的参数 =============================================

#define PEN_DOWN 0  //笔状态  下笔
#define PEN_UP 1    //笔状态  抬笔
...
TinyStepper_28BYJ_48
    m1;  //(7,8,9,10);  //M1 L步进电机   in1~4端口对应UNO  7 8 9 10
TinyStepper_28BYJ_48
    m2;  //(2,3,5,6);  //M2 R步进电机   in1~4端口对应UNO 2 3 5 6
